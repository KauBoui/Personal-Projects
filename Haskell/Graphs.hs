...
newtype Adjacency a = Adj [(a, [a])] deriving (Show, Eq)

newtype Friendly a = Edge [(a, a)] deriving (Show, Eq)
...
adjToGraph (Adj ((v, a):vs)) = Graph (v : xs) ((a >>= f) ++ ys)
   where
      f x = [(v, x) | not ((v, x) `elem` ys || (x, v) `elem` ys)]
...
friToGraph (Edge vs) = Graph xs ys
   where
      xs = foldr acc [] $ concatMap (\(a, b) -> [a, b]) vs
